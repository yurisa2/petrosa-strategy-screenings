...
def calc_screening(candles, periods):
    ...
    # verifying if the last close is higher and inclination is positive and yet if last high is higher than last but one high
    if (not sell_cond_1 and not sell_cond_2
            and close > last_high):

        ret['entry_value'] = low
        ret['disruption_value'] = low
        ret['stop_loss'] = high
        ret['take_profit'] = low - ((high - low) * 2)
        ret['type'] = 'VENDA'
        ret['direction'] = 'INFERIOR'
...
def entrypoint(request):
    ...
    try:
        ...
        if(result):
            ...
            new_line['movement'] = ''
            new_line['screening_type'] = 31
            new_line['screening_name'] = 'SETUP 9.2 VENDA'
